...
- Create elements using `document.createElement('div')`
- Add child to elements using `element.append`
- Add elements to html tags using `tag.appendChild`
  - `document.tag.append(element)` also works.
- Use `element.classList.add('cls')` to add a className to a referenced element
- Use `element.setAttribute('attr', 'attrValue')` to add arbitrary attributes to a referenced element, element id must be set this way
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=francisco.teixeira.de.barros%40outlook.com).
