...
- Babel compiles JSX code into regular JavaScript that is supported by the browsers.
    - We can import babel as a script in our HTML file to compile JSX code when it is served to the client.
        - This however, should only be done to `playground` and fiddle with code. It is a definite no no for production.
- You can use JSX instead of the raw React API to speed up development and keep the code cleaner.
- JSX supports JavaScript interpolation by using `{}` curly braces.
    - The interpolation should contain an expression (something that evaluates to a value)
    - JSX does not support JavaScript statements within interpolation, e.g.: imperative statements like `if-else return X`.
        - Notice that you can still do ternary statements, because they evaluate to a value. `true ? 'hello': 'bye'`.
- We can use the spread operator to pass propertities (`props`) to a given HTML tag such as `div`.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=francisco.teixeira.de.barros%40outlook.com).
