...
I learned how to use React functional components, Proptypes for validation and, React Fragments. 
Among these, I also learned to destrucutre `props` in the argument of a function.

React Fragments are usefull when you wish to Render a group of node elements without
adding extra nodes. For example, if you have a table element `<tr>`, the html expects
the child elements to be `<td>`, but this would not be possible with React components. 
React components return one and only one HTML nodes, with possibly N children. Consequently,
you would be able to return a `div` which wrapped those `<td>` elements, at best. This is not
ideal because the HTML `<tr>/<td>` relationship would be broken. For this scenario, the React 
library, offers a Fragments (`<React.Fragment> ... </React.Fragment>` or simply `<> ... </>`)
which allows you to return N node elements without containing them inside a parent node element.

See [here](https://www.educative.io/edpresso/what-are-react-fragments)
## Background
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=francisco.teixeira.de.barros%40outlook.com).
