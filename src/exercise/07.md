...
In this module I mostly learned of the short commings that I will face if I ever 
forget to set Key in a list item when using React. The problem is that React does not 
really have a way of knowing what is what once the initial list starts changing and 
the problem only gets worse if we have list items that have internal state.

The problem also exists in Vue, so I was more or less aware of this situation already, 
but still nice to know!

Notice that the `key identifier` problem is not just a problem when using `<li>`, but 
also when rendering any kind of array within the code.  For example, the array below
also needs key identifiers:

```jsx
<div>
  <h1>Without key property or with default key property (index) </h1>
  {items.map(item => (
    <input value={item.value}> onChange={doSomething(item)}
  ))}
</div>
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=francisco.teixeira.de.barros%40outlook.com).
